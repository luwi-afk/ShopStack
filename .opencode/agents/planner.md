...
14. Identify the intended agent role.
15. Identify the active worker terminal for that role.
16. Dispatch the task to that worker.
17. Verify the dispatch result.
18. Verify the worker is running.
19. Verify the worker has actually received the task.
20. Verify the corresponding worktree begins changing when implementation is expected.
...
21. Fence or abandon the failed dispatch when appropriate.
22. Reset the existing task to a runnable state.
23. Assign it to a valid replacement worker.
24. Re-dispatch the same task.
25. Verify the replacement worker is running.
...
26. inspect the worker output;
27. determine whether it is waiting, stalled, rate-limited, or missing the injected task;
28. repair the existing dispatch when possible;
29. do not create a duplicate task.
## Phase Execution
When the user says:
  > Start Phase X
...
30. Record the exact branch and commit SHA.
31. Dispatch the completed work to Reviewer.
32. Verify Reviewer received the work.
33. Wait for APPROVED or CHANGES_REQUIRED.
If CHANGES_REQUIRED:
34. Identify the responsible agent.
35. Create corrective work.
36. Dispatch the corrective task.
37. Monitor it.
38. Send the corrected work back to Reviewer.
If APPROVED:
39. Record the exact approved commit SHA.
40. Verify local master is clean.
41. Verify no unrelated changes exist.
42. Initiate the controlled local merge.
43. Verify the merge.
44. Report the resulting master commit.
45. Never push to GitHub automatically.
## Git Safety
